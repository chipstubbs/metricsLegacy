...
if(!empty($yearToDateDollar)){
    if ($this->session->userdata('year_to_view') == '2015') { $yearToDatePercent = ($completedAnnuities / $annualGoal); } // No longer using year to date dollar. Which is only for current year
    elseif ($this->session->userdata('year_to_view') == '2016') { $yearToDatePercent = ($completedAnnuities / $yearToDateDollar); }
                ...
                <div class="stdpad"><h2>AUM</h2><a href="<?=base_url()?>reports/view/<?=$aumsheet[0]->id;?>">
                      <ul class="eventlist">
                          ...
                          <li class="dark"><?php $notsubmittedAum = 0;
                              foreach ($production as &$pr) {
                                  if(empty($pr->production_submitted) && $pr->production_type == 'aum' && empty($pr->prem_paid_month)) {
