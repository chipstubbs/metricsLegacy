...
$whereiis = $this->uri->uri_string();
$this->session->set_userdata('refer_from', $whereiis);
 ?>
<!DOCTYPE html>
<html lang="en">
  <head>
...
<div id="mainwrapper">
 <a href="#" class="menu-trigger"><i class="fa fa-bars visible-xs"></i></a>
    ...
    <?php foreach ($widgets as $key => $val) {
        ...
        <?php foreach ($user_online as $value):
                ...
                if($value->userpic != 'no-pic.png'){
                 ...
                 ?>" width="50px" /> <?php echo $value->firstname." ".$value->lastname;?> </p>
                    </a>
                    <!-- <div class="submenu">
                        <ul>
    ...
    <div class="content-area" onclick="">
      ...
      <label style="height: 54px; line-height: 24px; float: left; color: #218ee0; padding: 20px;"><?=$company->primary_contact.' - '.$company->name;?></label>
      <?php } ?>
      <div class="topbar">
        <label style="margin-right: 20px;"><?php echo 'Production Year - '.$this->session->userdata('year_to_view'); ?></label>
      <?php
      ...
      <img class="img-circle topbar-userpic hidden-xs" src="<?=$userimage;?>" height="50px">
      ...
      <span class="hidden-xs"><a href="http://metrics.advisorsacademy.com/files/media/Metrics_Manual.pdf" target="_blank"><i class="fa fa-file-pdf-o"></i> Manual </a></span>
  ...
  <script type="text/javascript" charset="utf-8">
    ...
    // ToolTips
      $(document).ready(function(){
        ...
        $('#EventAverages, #ReferralEventAverages, #OverallEventAverages').DataTable( {
            "search": {
                "smart": true
            },
            "order": [[ 2, "asc" ]],
     ...
     $('#hotProspectsTable, #hotClientsTable').addClass('table').DataTable( {
      ...
      // Outside of doc load
            ...
            //         mPadd = 240, // Mousemove Padding
            //         damp = 20, // Mousemove response softness
            //         mX = 0, // Real mouse position
            //         mX2 = 0, // Modified mouse position
            //         posX = 0,
            //         mmAA = eleW - (mPadd * 2), // The mousemove available area
            ...
            //     //
            // };
            // var scrbdy = $("#production_wrapper .dataTables_scrollBody");
