	...
	<div class="col-md-10 col-xs-12">
		...
		<table class="table" id="EventAverages" width="100%">
			...
			<tr>
				...
				<td><?php echo '&dollar;'.number_format($client_avgcase, 2).'&nbsp; &nbsp; &nbsp;<span class="avg-percent">(&nbsp;'.$client_annuity_avg.'&nbsp;)</span>'; ?></td>
				<td><?php echo '&dollar;'.number_format($client_annuity, 2); ?></td>
			</tr>
			<!-- Referral averages -->
			<tr>
				<td>Unsolicited Referrals</td>
				<td><?php echo $num_unsolicited; ?></td>
				<td></td>
				...
				<td><?php echo '&dollar;'.number_format($referral_avgcase, 2).'&nbsp; &nbsp; &nbsp;<span class="avg-percent">(&nbsp;'.$referral_annuity_avg.'&nbsp;)</span>'; ?></td>
				<td><?php echo '&dollar;'.number_format($referral_annuity, 2); ?></td>
			</tr>
			<tr>
				<td>Solicited Referrals</td>
				<td><?php echo $num_solicited; ?></td>
				<td></td>
				<td><?php echo $solicited_referral_appts.' <span class="avg-percent">(&nbsp;'.$solicited_referral_appts_ratio.'&nbsp;)</span>'; ?></td>
				<td><?php echo $solicited_referral_kept_appts.' <span class="avg-percent">(&nbsp;'.$solicited_referral_kept_ratio.'&nbsp;)</span>'; ?></td>
				<td><?php echo $solicited_referral_closed.' <span class="avg-percent">(&nbsp;'.$solicited_referral_close_ratio.'&nbsp;)</span>'; ?></td>
				<td></td>
				<td><?php echo '&dollar;'.number_format($solicited_referral_gross, 2); ?></td>
				<td><?php echo '&dollar;'.number_format($solicited_avgcase, 2).'&nbsp; &nbsp; &nbsp;<span class="avg-percent">(&nbsp;'.$solicited_referral_annuity_avg.'&nbsp;)</span>'; ?></td>
				<td><?php echo '&dollar;'.number_format($solicited_referral_annuity, 2); ?></td>
			</tr>
			<?php if(is_object($avgs)) {
				foreach ($avgs as $e => $avg) { ?>
					<tr>
						...
						<td style="display:none;"> <?php echo 'pclosed = '.$avg->pclosed.' ka = '.$avg->ka; ?> </td>
					</tr>
				<?php }
			} else {
	...
	<div class="col-xs-12 col-sm-12">
		...
		?>
	<div class="table-head">Events / Lead Source</div>
	<div class="table-div">
	<table class="table nowrap" id="eventOverview" rel="<?=base_url()?>" cellspacing="0" cellpadding="0" >
			...
			<tbody>
			<?php
				foreach ($eventprojects as $value):
					if ($value->name == '6') {
				$workers = array();
				...
				if (!empty($kept_appointments)) { $assets_ratio = $has_assets/$kept_appointments; }
				...
				if (!empty($prospects_closed) && !empty($has_assets)) { $closing_ratio = $prospects_closed/$has_assets; } 
...
$(document).ready(function(){
						...
						//Auto scroll over events
						//Table Mouse Scroll
						var scrollHwithMouse = function(ELE) {
							var $ele = $(ELE),
								eleW = $ele.outerWidth(true),
								eleSW = $ele[0].scrollWidth,
								wDiff = (eleSW / eleW) - 1, // widths difference ratio
								mPadd = 40, // Mousemove Padding
								damp = 20, // Mousemove response softness
								mX = 0, // Real mouse position
								mX2 = 0, // Modified mouse position
								posX = 0,
								mmAA = eleW - (mPadd * 3), // The mousemove available area
								mmAAr = eleW / mmAA; // get available mousemove fidderence ratio
							//
							...
							setInterval(function() {
								...
								$ele.scrollLeft(posX * wDiff);
							}, 10);
							//
						};
						var scrbdy = $("#eventOverview_wrapper .dataTables_scrollBody");
  						// scrollHwithMouse(scrbdy);
