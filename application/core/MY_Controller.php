	...
	function __construct()
	{
		...
		if($this->user || $this->client){
			...
			if($this->user){
				...
				// $this->view_data['sticky'] = Project::find_by_sql("select distinct(projects.name), projects.id, projects.progress, projects.event from projects, project_has_workers where projects.sticky = 1 AND projects.id = project_has_workers.project_id AND project_has_workers.user_id=".$this->user->id);
				$this->view_data['sticky'] = Project::find_by_sql("select distinct(name), id, progress, event FROM projects WHERE sticky = 1 AND company_id = ".$this->user->company_id);
				//Set admins company view
				if($this->user->admin == 1){ $this->view_data["company"] = Company::find(array('conditions' => 'inactive=0 and id='.$this->user->company_id)); }
