...
class Settings extends MY_Controller
{
	...
	function users()
	{
        ...
        /*$query = $this->db->get_where('users', array('id' => $companyid), $limit, $offset);*/
        //Get company name and phone number
//        $sql = $this->db->query("SELECT companies.name as Company, clients.phone as Phone FROM companies INNER JOIN clients ON clients.company_id = companies.id INNER JOIN users ON users.client_id = clients.id WHERE users.id = ".$this->user->id);
//        foreach ($sql->result() as $row)
//        {
//           $this->load->view('qcompany',  $row->Company;)
//           $this->load->view('qphone',  $row->Phone;)
//        }
//        $this->load->view('queryresult', $query[0]);
//        foreach ($query->result() as $row)
//        {
//            $thetest[] = $row->title;
//            $this->view_data['phone'] = $row->title;
//
//        }
        //$row->title;
        ...
        /*$this->view_data['getcompany'] = User::count(array('conditions' => 'user_id = '.$this->user->id.' and status = "open"'));*/
        //End get company name and phone number
        $this->view_data['companies'] = Company::find('all',array('conditions' => array('inactive=?','0')));
	...
	function user_create()
	{
		...
		}else
		{
			...
			$this->view_data['companies'] = Company::all();
	...
	function user_update($user = FALSE){
		...
		unset($_POST['file-name']);
 		...
 		if($_POST['admin'] == "0" && $_POST['username'] == "Admin"){ $_POST['admin'] = "1";}
			// $userintocompany = $_POST['company_id'];
			// $companies = Company::find('all', array('conditions' => array('id = ?', $userintocompany)));
			// $companies->update_attributes(array('user_id' => $user_id));
